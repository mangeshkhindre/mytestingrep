...
'@ TestCase:  PTAC-1564 Verify Appraisal Order with Loan type as 'HELOC'
'@ Test Automation JIRA Task: PTAC-1554 BusinessRules_AppraisalOrderLoanTypeAsHELOC
'@ TestData: 
...
'@ TestSteps:
   '1 Login to Encompass with Admin user
      ...
      'select loan Type as 'HELOC',select Ordering Optional as 'Approved Appraisers Only' and select 'Loan Officer' Persona and check 'Enable Ordering' check box
...
'***************************************************************************************************
Set objData = FRM_DS_GetTestData("BusinessRule_PersonaAccessToFields", "AppraiserOrderDetails", "PTAC-1564")
strExecutionFlag = FRM_DS_GetValue(objData, "ExecutionFlag")
If strExecutionFlag = "Y" Then

FRM_Logger_ReportStepEvent "Start Test Case : PTAC-1564","Verify Appraisal Order with Loan type as 'HELOC'", Null
...
BR_AdminUser_ChangeSettings "PTAC-1564"
...
strLoanNumber = BR_CreateLoanWithBorrowerSummaryOriginationInfo("PTAC-1564")
Wait g_ShortWaitMedium + g_ShortWaitMedium 'Due To Sync Issue We Are Explicitly Passing Wait Statement
BR_OrderAppraisal_NonAdmin "PTAC-1564",strLoanNumber
BIZ_Loan_Exit(False)
BIZ_Nav_SelectHomeTab

'====== Logout with ANon dmin User ======
BIZ_Login_UserLogout()
