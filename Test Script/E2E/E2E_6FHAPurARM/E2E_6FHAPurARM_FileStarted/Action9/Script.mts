...
'@ TestCase : PTAC-1869 File started 7-Get Rate Locked by Secondary locking user
'@ Test Automation JIRA Task: PTAC-2011 E2E_6FHAPURARM_Filestarted
'@ TestData:
   'Global_Data, Login, E2E_Secondary
'@ Pre-conditions: 
'@ Description:  
'@ TestSteps:
...
FRM_Logger_ReportStepEvent "Start Test Case - PTAC-1869","File started 7- Get Rate Locked by Secondary locking user", Null

Dim blnDaysExists, SnapShotGrid, SecondaryLockTool, MainPage, intNumberOfLockRequests, objData, strLoanNumber, objSnapShotGrid, objSecondaryLockTool, objRequest
Dim ObjLockRequestForm
...
If (intNumberOfLockRequests > 0) Then 
	...
	GUI_Object_WaitTillExistX  objSecondaryLockTool.SwfButton("swfname:=btnBuySidePricing"), g_LongWaitTime
	...
	If (SwfWindow("swfname:=MainForm").SwfWindow("swfname:=UserPasswordDialog").Exist(10)) Then 
 	...
 	Else
		GUI_Object_WaitTillExistX SwfWindow("swfname:=MainForm").SwfWindow("swfname:=RequestDialog").SwfButton("swfname:=btnContinue"), 120
		GUI_Object_ValidateExists SwfWindow("swfname:=MainForm").SwfWindow("swfname:=RequestDialog").SwfButton("swfname:=btnContinue"), 60, "product and pricing window is open"
	...
	Else
	    FRM_Logger_ReportFailEvent "Lock Request - Update Product Pricing", "no. of days is not displayed next to Lock Icon", Null
	End If 
Else
