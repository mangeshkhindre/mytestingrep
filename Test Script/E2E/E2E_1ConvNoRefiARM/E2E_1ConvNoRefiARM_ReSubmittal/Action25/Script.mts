...
'@ TestCase: 
   '1 PTAC-1397 - CONVNOCASHREFIARM-Resubmittal 2- Get Rate Locked
'@ Test Automation JIRA Task: PTAC-1786 E2E_ConvNoRefiARM_ReSubmittal
'@ TestData: 
	'Loans, LoanTemplate and E2E_SecondaryMarketing
	'Global_Data, Login and E2E_Secondary
'@ Pre-conditions: 
'@ Description:  
'@ TestSteps:
   '1 Login to Encompass as secondary user with the credentials provided in the test data column.
   '2 Select your loan by your loan number and open the loan.
   '3 Under the lock request snapshot select the first line by double clicking on it.
   '4 Click Copy from request and then click Copy from buy side button and click Lock and Confirm button
   '5 Click confirm in the current loan data window
   '6 Click Update button in the pricing import window
   '7 Click ok in the Encompass lock confirmation window
'@ ExpectedResult:
   '1 Should be able to login. Loan should open with the secondary registration page.
   '2 Loan should open with secondary registration Page
   '3 Secondary lock tool will open
   '4 Compare with current loan data window will open
   '5 Encompass Product and pricing service pricing import window will open
   '6 Lock has been successfully confirmed window will open
   '7 In the secondary registration window status of the loan will be locked
'***************************************************************************************************
FRM_Logger_ReportStepEvent "Start Test Case: PTAC-1397","Test Case Name - CONVNOCASHREFIARM-Resubmittal 2- Get Rate Locked", Null
...
If(intNumberOfLockRequests > 0) Then 
   ...
   If(SwfWindow("swfname:=MainForm").SwfWindow("swfname:=UserPasswordDialog").Exist(10)) Then 
	  ...
	  'Click on Rate Link
	  BIZ_LockRequestForm_ImportPricing "E2E_ConvNoRefiARM", "Float"
   ...
   Else 
   	   FRM_Logger_ReportFailEvent "Lock Request - Update Product Pricing","Lock icon has no number of days next to it", Null
   End If 
Else
