...
'@ TestCase : PTAC-2152 File started 6-Get Rate Lock request and LO comp plan and Float.
'@ Test Automation JIRA Task: PTAC-2408 - E2E_8VAPURARM_FileStarted
'@ TestData: 
    '1 Tools_LockRequestForm, SetRateLockRequest and E2E_VAPURARM
'@ Pre-conditions: 
'@ Description: Get Rate Lock and LO comp plan and Float.
'@ TestSteps:
	'01 Login to Encompass as Loan officer with the provided credentials. Click lock icon in the header section 
	...
	'09 click exit loan button in the popup window.
	'10 In pipeline select the same loan by double clicking on it 
'@ ExpectedResult:  
...
FRM_Logger_ReportStepEvent "Start Test Case PTAC-2152","File started 6-Get Rate Lock request and LO comp plan and Float.", Null

Dim LockRequestedLabelText, strLoanNumber, blnProductNPricingReceived

If ((BIZ_LockRequestForm_ProductNPricingFromGetPricing("E2E_VAPURARM", "Float") = True)) Then 
 	 'Go to Pipeline Page
