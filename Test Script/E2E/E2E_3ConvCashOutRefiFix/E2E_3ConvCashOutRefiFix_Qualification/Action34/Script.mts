...
'@ TestCase : PTAC-3308 CONVCASHOUTREFIFIX Qualification 3 - Get Rate Locked
'@ Test Automation JIRA Task: PTAC-3372 E2E_3CONVCASHOUTREFIFIX_Qualification
'@ TestData: 
	'1 Global_Data, Login, E2E_Secondary
	'2 Tools_LockRequestForm, SetRateLockRequest, E2E_CONVCASHOUTREFIFIX
'@ Pre-conditions: 
'@ Description:  
'@ TestSteps:
	...
	'2 Select your loan by your loan number and open the loan.
      '1 Select your loan by your loan number and open the loan. 
...
'@ ExpectedResult:
	'1 Should be able to sign in as secondary user . Loan should open with secondary registration snapshot list 
...
FRM_Logger_ReportStepEvent "Start Test Case : PTAC-3308","CONVCASHOUTREFIFIX Qualification 3 - Get Rate Locked", Null

Dim blnDaysExists, objSnapShotGrid, objSecondaryLockTool, objMainPage, intNumberOfLockRequests,objData, strLoanNumber
Dim objRequest, ObjLockRequestForm
...
If (intNumberOfLockRequests > 0) Then 
	...
	If (SwfWindow("swfname:=MainForm").SwfWindow("swfname:=UserPasswordDialog").Exist(10)) Then 
		...
		BIZ_LockRequestForm_ImportPricing "E2E_CONVCASHOUTREFIFIX", "Float"
