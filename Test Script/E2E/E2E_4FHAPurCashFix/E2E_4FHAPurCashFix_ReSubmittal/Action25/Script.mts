...
'@ TestCase : PTAC-3177 FHAPURCHASEFIX- Resubmittal 2- Get Rate Locked
...
'@ TestData:
    ...
    '3 Tools_LockRequestForm, SetRateLockRequest, E2E_FHAPURCASHFIX    
'@ Pre-conditions: 
'@ Description:  
'@ TestSteps:
    ...
    '2 Select your loan by your loan number and open the loan.
      'Under the lock request snapshot select the first line by double clicking on it.
      'Click Copy from request and then click Copy from buy side button and click Lock and Confirm button
      'Click confirm in the current loan data window
      'Click Update button in the pricing import window
      'Click ok in the Encompass lock confirmation window
'@ ExpectedResult:
    '1 Should be able to login.Loan should open with the secondary registration page.
    '2 Loan should open with the secondary registration page.
      'Secondary lock tool will open
      'Compare with current loan data window will open
      'Encompass Product and pricing service pricing import window will open
      'Lock has been successfully confirmed window will open
      'In the secondary registration window status of the loan will be locked
'***************************************************************************************************

FRM_Logger_ReportStepEvent "Start Test Case : PTAC-3177","FHAPURCHASEFIX- Resubmittal 2- Get Rate Locked", Null

Dim objData, strLoanNumber, blnDaysExists, objSnapShotGrid,  objMainPage, intNumberOfLockRequests, objSecondaryLockTool, objRequest
...
If (intNumberOfLockRequests > 0) Then 
    ...
    If (SwfWindow("swfname:=MainForm").SwfWindow("swfname:=UserPasswordDialog").Exist(10)) Then 
        ...
        'Click on Rate Link and Clicks on Float Button
        BIZ_LockRequestForm_ImportPricing "E2E_FHAPURCASHFIX", "Float" 
	...
	If (GUI_Object_IsExistX(objSecondaryLockTool.SwfWindow("swfname:=LockSnapshotCompareForm").SwfButton("swfname:=btnConfirm"), 60))Then 
