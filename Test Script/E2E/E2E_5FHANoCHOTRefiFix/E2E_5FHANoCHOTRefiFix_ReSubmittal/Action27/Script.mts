...
'@ TestCase : PTAC-1990 FHANOCHOTREFIFIX Resubmittal 2- Get Rate Locked  
'@ Test Automation JIRA Task: PTAC-2704 E2E_FHANoCHOTRefiFix_ReSubmittal
'@ TestData: 
	'1 Loans, LoanTemplate, E2E_SecondaryMarketing
    '2 eFolder_Tab, SelectPackageTypeAndPlanCode, E2E_FHANoCHOTRefiFix
    '3 Tools_LockRequestForm, SetRateLockRequest, E2E_FHANoCHOTRefiFix  
    '4 Global_Data, Login and E2E_Secondary
'@ Pre-conditions: 
'@ Description:  
'@ TestSteps:
   ...
   '2 1 Select your loan by your loan number and open the loan. 
     '2 Under the lock request snapshot select the first line by double clicking on it.
...
'@ ExpectedResult:
   '1 Loan should open with the secondary registration page.
...
FRM_Logger_ReportStepEvent "Start Test Case: PTAC-1990","FHANOCHOTREFIFIX Resubmittal 2- Get Rate Locked", Null
...
If (intNumberOfLockRequests > 0) Then 
   ...
   If (SwfWindow("swfname:=MainForm").SwfWindow("swfname:=UserPasswordDialog").Exist(10)) Then 
	   ...
	   'Click on Rate Link
      	BIZ_LockRequestForm_ImportPricing "E2E_FHANoCHOTRefiFix", "Float"
