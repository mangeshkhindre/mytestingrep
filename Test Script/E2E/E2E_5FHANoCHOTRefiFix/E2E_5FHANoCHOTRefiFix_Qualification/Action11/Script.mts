...
'@ TestCase : PTAC-1818 FHANOCHOTREFIFIX - Qualification 5 - Order Encompass compliance service
'@ Test Automation JIRA Task: PTAC-2447 E2E_5FHANoCHOTRefiFix_Qualification
'@ TestData: Loans, Milestone and E2E_FHANOCHOTREFIFIX_Qualification
'@ Pre-conditions: 
'@ Description:  
'@ TestSteps:
...
FRM_Logger_ReportStepEvent "Start Test Case: PTAC-1818", "FHANOCHOTREFIFIX - Qualification 5 - Order Encompass compliance service", Null

Dim objMainForm, objAvailableUsers, objLoanPage, objData, intComplianceReviewStatus
BIZ_Nav_SelectLoanTab()

Set objMainForm 		= SwfWindow("swfname:=MainForm")
Set objAvailableUsers 	= objMainForm.SwfWindow("swfname:=.*SelectionDialog").SwfObject("swfname:=gvUsers")
Set objLoanPage 		= objMainForm.SwfWindow("swfname:=MainScreen").SwfWindow("swfname:=LoanPage")
Set objData 			= FRM_DS_GetTestData("Loans", "Milestone", "E2E_FHANOCHOTREFIFIX_Qualification")
...
If (BIZ_Loan_VerifyIfMilestoneIsCompletedForLoan("Qualification finished")) Then 
	BIZ_Forms_Open "Borrower Summary - Origination"
	GUI_WebEdit_Set SwfWindow("swfname:=MainForm").Page("title:=.*","index:=0").WebEdit("html id:=TextBox10"), "MS2Complete_FHANoCHOTRefiFix"
End If
